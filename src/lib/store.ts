...
import type { BusinessProfile, GrantApplication, ApplicationStatus } from "./types";
    ...
    (set, get) => ({
      ...
      startApplication: (grantId) => {
        const existing = get().applications.find(
          (a) => a.grantId === grantId && a.status !== "declined",
        );
      ...
      setSection: (appId, sectionId, content) =>
        set((s) => ({
          applications: s.applications.map((a) =>
            a.id === appId
              ? {
          ...
          applications: s.applications.map((a) =>
            a.id === appId
              ? {
                  ...a,
                  updatedAt: new Date().toISOString(),
                  checkedDocs: a.checkedDocs.includes(doc)
                    ? a.checkedDocs.filter((d) => d !== doc)
                    : [...a.checkedDocs, doc],
                }
              : a,
          ),
      ...
      setStatus: (appId, status) =>
        set((s) => ({
          applications: s.applications.map((a) =>
            a.id === appId ? { ...a, status, updatedAt: new Date().toISOString() } : a,
          ),
        })),
