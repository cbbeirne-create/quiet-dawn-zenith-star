...
function AppWorkspace() {
  ...
  async function draft() {
    ...
    if (!isPending && !user) { setErr("Sign in to draft with AI."); return; }
    ...
    catch (e) { setErr(unauthorized(e) ? "Sign in to draft with AI." : "Draft failed."); }
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[240px_1fr]">
        <aside className="space-y-1">
          ...
          <div className="pt-6"><p className="text-xs uppercase tracking-wide text-muted">Your files</p><label className="mt-2 flex cursor-pointer items-center justify-center rounded-md border border-dashed border-border px-3 py-3 text-sm hover:bg-surface"><input type="file" multiple className="sr-only" onChange={(e) => addFiles(e.target.files)} />Add supporting files</label><ul className="mt-2 space-y-1 text-xs text-muted">{(app.uploadedDocuments ?? []).map((d) => <li key={d.id} className="flex items-center justify-between gap-2"><span className="truncate">{d.name}</span><button type="button" className="text-danger" onClick={() => removeUploadedDocument(app.id, d.id)}>Remove</button></li>)}</ul><p className="mt-2 text-[11px] text-subtle">File metadata is saved locally; file contents are not uploaded.</p></div>
        </aside>
        <div>
          {section && <><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-display text-2xl">{section.title}</h2><p className="text-sm text-muted">{hint}</p></div><Button variant="ink" onClick={draft} disabled={busy}>{busy ? "Drafting…" : "Draft with AI"}</Button></div><p className="mt-2 text-xs text-subtle">AI draft — edit before you submit. Daily cap applies when signed in.</p>{err && <p className="mt-2 text-sm text-danger">{err} {!user && <Link to="/login" className="underline">Sign in</Link>}</p>}<Textarea className="mt-4 min-h-72" value={section.content} onChange={(e) => setSection(app.id, section.id, e.target.value)} placeholder="Write this section, or sign in and ask AI to draft from your company profile." /></>}
