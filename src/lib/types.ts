...
export type Grant = {
  id: string; name: string; shortName: string; agency: Agency; category: string; amountLabel: string; amountMax: number;
  coFundRate: number; deadlineKind: DeadlineKind; deadlineNote: string; summary: string; covers: string[]; nextSteps: string[];
  documents: string[]; sections: { id: string; title: string; hint: string }[]; winRate: number; winBySector: Partial<Record<Sector, number>>;
  typicalAward: string; difficulty: "straightforward" | "moderate" | "competitive"; officialUrl: string; rules: EligibilityRule[]; goals: Goal[];
  verificationStatus?: VerificationStatus; lastVerifiedAt?: string; verificationNote?: string;
};
export type BusinessProfile = {
  companyName: string; form: CompanyForm; stage: CompanyStage; employees: number; tradingMonths: number; region: Region;
  sector: Sector; goals: Goal[]; eiClient: boolean; description: string; turnoverEur?: number | null; county?: string;
};
...
export type GrantApplication = {
  id: string; grantId: string; status: ApplicationStatus; sections: AppSection[]; checkedDocs: string[]; uploadedDocuments?: UploadedDocument[];
  completedSteps?: string[]; notes: string; createdAt: string; updatedAt: string;
};
