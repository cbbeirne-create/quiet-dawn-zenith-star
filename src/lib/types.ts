...
export type EligibilityRule = {
  id: string;
  label: string;
  test: (p: BusinessProfile) => boolean;
  knockout?: boolean;
};
...
export type Grant = {
  ...
  winRate: number;
  winBySector: Partial<Record<Sector, number>>;
  typicalAward: string;
  difficulty: "straightforward" | "moderate" | "competitive";
  officialUrl: string;
...
export type MatchResult = {
  ...
  score: number;
  eligible: boolean;
  reasons: { ok: boolean; label: string; knockout?: boolean }[];
  fitNotes: string[];
};
