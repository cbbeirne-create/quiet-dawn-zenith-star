    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Select value={app.status} onChange={(e) => setStatus(app.id, e.target.value as ApplicationStatus)} className="w-40">{STATUSES.map((s) => <option key={s} value={s}>{s.replace("_", " ")}</option>)}</Select>
          <Badge>{grant?.amountLabel}</Badge><Button variant="outline" size="sm" onClick={download}>Download .txt</Button>
          <Link to="/pipeline/$id/print" params={{ id: app.id }}><Button variant="outline" size="sm">Print / PDF</Button></Link>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-lg border border-border bg-surface p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div><p className="text-xs uppercase tracking-wide text-muted">Application readiness</p><p className="mt-1 font-display text-3xl">{readiness.score}%</p></div>
          <p className="text-sm text-muted">{readiness.completedSections}/{readiness.totalSections} sections · {readiness.readyDocuments}/{readiness.totalDocuments} required documents</p>
        </div>
